...
- Importing transcript-level counts from Salmon and aggregating to gene-level
    - Explain difference between CPM and TPM
-->
...
However, Quarto provides some additional functionality that can be very useful.
...
::: {.callout-tip}
Code chunks require a header, which in its most simple form exists as `{r}`.
I have assigned a label to the above code chunk, which is best practice but completely optional.
...
For any packages that require installation, use the RStudio console to do so, e.g. `BiocManager::install("tidyverse")`.
...
```{r meta}
meta <- read_tsv(
    here("smk-rnaseq-counts-1.2.2/config/samples.tsv")
) %>%
    mutate(
...
Gene-level count data was previously generated by `featureCounts` using the Snakemake workflow.
Normally I do not push `featureCounts` output files to GitHub, as they often border the file size limit of 50MB.
Instead I transfer them manually from the Phoenix HPC to my local device.
This means the count data is not backed up, but, the Snakemake code still is.
...
For the sake of this example the data is included in the repository.
...
The [edgeR User's Guide](https://www.bioconductor.org/packages/devel/bioc/vignettes/edgeR/inst/doc/edgeRUsersGuide.pdf) is filled with extensive information and use cases for the methodology in the following sections.
:::
...
Here we can see that prior to filtering, the majority of genes in each sample had very low (or no) counts.
...
This is helpful for quality assessment, whereby outlier samples can be identified.
...
Normalisation factors are used to calculate effective library sizes.
...
I find [this paper](10.12688/f1000research.27893.1) an incredibly useful resource to construct both simple and complex design matrices.
...
I'll attempt to provide a simplified summary of the implemented methodology.
...
We now have gene-level assigned *p*-values, as well as other relevant stats in our `tt` variable.
...
A QL dispersion plot can be inferred similarly to that of a BCV plot.
...
Rendering the entire set of results in our report will impact the file size, so let's just take the top 1000.
...
I always end my documents by printing the session information.
This lists version information about the operating system, R and its attached packages, which has definitely helped me in the past when things have broken.
